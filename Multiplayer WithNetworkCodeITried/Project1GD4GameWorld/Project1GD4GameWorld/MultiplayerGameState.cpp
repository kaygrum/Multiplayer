...
{
	mBroadcastText.setFont(context.fonts->get(FontIDs::Main));
	mBroadcastText.setPosition(1024.f / 2, 100.f);

	mPlayerInvitationText.setFont(context.fonts->get(FontIDs::Main));
	...
	mFailedConnectionText.setFont(context.fonts->get(FontIDs::Main));
...
bool MultiplayerGameState::handleEvent(const sf::Event& event)
{
	...
	if (event.type == sf::Event::KeyPressed)
	{
		...
		else if (event.key.code == sf::Keyboard::Escape)
		{
			...
			requestStackPush(StateIDs::NetworkPause);
		}
	}
...
void MultiplayerGameState::handlePacket(sf::Int32 packetType, sf::Packet& packet)
{
	...
	case Server::SpawnSelf:
	{
		...
		mPlayers[characterIdentifier].reset(new Player(&mSocket));
	...
	{
		requestStackPush(StateIDs::MissionSuccess);
	} break;
